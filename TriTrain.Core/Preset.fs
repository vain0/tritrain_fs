...
module Scope =
  let self    = Self
  let homeFwd = AbsScope (set [Fwd], Set.empty)
  let oppoFwd = AbsScope (Set.empty, set [Fwd])
  let homeBwd = AbsScope (set [Lft; Rgt], Set.empty)
  let oppoBwd = AbsScope (Set.empty, set [Lft; Rgt])
  let homeRgt = AbsScope (set [Rgt], Set.empty)
  let oppoRgt = AbsScope (Set.empty, set [Rgt])
  let homeAll = AbsScope (set (Vertex.all), Set.empty)
  let oppoAll = AbsScope (Set.empty, set (Vertex.all))
